...
createSSCs <- function(gridded_tuple, daymet_data, min_per_cell_n){
    ...
    gridded_tuple[[1]]$cell <- gridded_tuple[[1]]$grid_id
    # NEW - Akshay approach
    # format sampling effort
    sampling_effort <- gridded_tuple[[1]] %>%
    ...
    # bind pred into data
    pred <- predict(m, gridded_tuple[[1]])
    pred <- 1 / as.numeric(pred)
    gridded_tuple[[1]]$inverse_pred_effort <- pred
    ...
    sscs <- gridded_tuple[[1]] %>%
      ...
      dplyr::summarise(n = n(),obs=list(local_hour),tile=Mode(daymet_tile),median_hr_bs=my_bootstrap_median(local_hour,inverse_pred_effort)) %>%
      dplyr::filter(n > ssc_n_obs_threshold)
