...
downloadSaveDaymetData <- function(cellsize_km,download_path){
    ...
    write.csv(points,paste0(download_path,"/daymet_raw/points/points_for_daymet.csv"),row.names = FALSE)

    # download 16500 (max) tiles from these points
    download_daymet_batch(
        file_location = paste0(download_path,"/daymet_raw/points/points_for_daymet.csv"),
        start = 2020,
        end = 2021,
        internal = FALSE,
        path = paste0(download_path,"/daymet_raw"),
        simplify = FALSE
    )
